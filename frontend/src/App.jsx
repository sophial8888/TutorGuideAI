...
export default function App() {
  ...
  const endSession = async () => {
    ...
    try {
      const res = await fetch("/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: [{ role: "user", content: `Generate a post-session reflection report with four labeled sections:\n1. SESSION SUMMARY\n2. STUDENT LEARNING INSIGHTS\n3. COACHING FEEDBACK\n4. TOOLS USED\nKeep each 2-4 sentences.\nData:\n${sessionSummary}` }] }) });
  ...
  const generatePlan = async () => {
    ...
    try {
      const res = await fetch("/generate-plan", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ subject: selectedSubject, topic: selectedTopic }) });
      const data = await res.json();
  ...
  const callAI = async (prompt, label, toolType) => {
    ...
    try {
      const res = await fetch("/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: [{ role: "user", content: prompt }], ...getSessionContext() }) });
      const data = await res.json();
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch("/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: newMessages, ...getSessionContext() }) });
      const data = await res.json();
      const reply = data.reply || "Error: " + (data.error || "something went wrong");
      if (reply.includes("VISUAL:")) setToolsUsed((prev) => ({ ...prev, graphs: prev.graphs + 1 }));
      setMessages([...newMessages, { role: "assistant", content: reply }]);
